...
const DOWN = 3
const NONE = -1
const RM_TYPE_START = "START"
const RM_TYPE_CHEST = "CHEST"
const RM_TYPE_FIGHT = "FIGHT"
const RM_TYPE_SHOP = "SHOP"
const RM_TYPE_REST = "REST"
const RM_TYPE_TRAP = "TRAP"
const RM_TYPE_ELITE = "ELITE"
...
func _gen_room(room:Dungeon_Room, level:int):
	var nb_door = _get_nb_room(level) 
	var doors = [0, 1, 2]
	...
	for i in range(room.rooms.size()):
		if room.rooms[i] != null:
			_gen_room(room.rooms[i], level+1)

func _get_room_type(level:int) -> String:
	var x = rng.randf()
	match level:
		1, 2:
			if x <= 0.6:
...
func _get_room_type2(level:int) -> String:
	var x = rng.randf()
	match level:
		1, 2, 3:
			if x <= 0.1:
...
var current_room:Dungeon_Room = null

func new_dungeon():
	dungeon = Dungeon_Room.new(null, "start_point", 0)
	dungeon.type = RM_TYPE_START
	current_room = dungeon
	_gen_dungeon()

func set_current_room(direction):
	if direction == DOWN :
		current_room = current_room.parent
	else:
		current_room = current_room.rooms[direction]
